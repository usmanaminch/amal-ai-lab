  ...
  try {
    ...
    const response = await client.responses.create({
      ...
      input: [
        ...
        { role: "user", content: `Create one fresh essay prompt for ${level}. Essay type: ${type}. Topic preference: ${topic}. The student has a ${characterLimit} character limit, so make the prompt fit that writing length. Do not ask for private personal information. Return JSON only with title, prompt, type, and skills array of 3 items.` }
      ],
